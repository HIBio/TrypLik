...
numbers (specifically the numbers of $\alpha$ alleles alleles and $\beta$
...
it is much more challenging to estimate copy numbers from standard
...
total number of $\alpha$ alleles, total number of $\beta$ alleles, and total of
$\beta$ frameshift alleles. The method accounts for dependence across read
counts (caused by the same $2 \times 150$ bp read generally overlapping multiple
of the regions used to distinguish between alleles) and also uses prior
...
tryptase alignment, the Maun et al. (2019) tryptase alignment, and
...
$$
 \textrm{Pr} \bigl(\textrm{data}\ |\ \beta\textrm{III}^{FS}\, \textrm{heterozygote}\bigr) = \binom{64}{28} (0.25)^{28}(0.75)^{36},\ \textrm{and}
$$
$$
 \textrm{Pr} \bigl(\textrm{data}\ |\ \beta\textrm{III}^{FS}\, \textrm{homozygote}\bigr) = \binom{64}{28} (0.5)^{28} (0.5)^{36}
$$
